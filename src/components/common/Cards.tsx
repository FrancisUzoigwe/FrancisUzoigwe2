...
const Cards: React.FC<iCard> = ({ github, url, image, name, description }) => {
    return (
        <>
            <div className="border min-h-[400px] flex flex-col items-center rounded-lg pb-4 bg-white">
                <div className="w-[90%]  h-[200px] rounded-lg mt-5 relative ">
                    <Link to={`${github}`}>
                        <div className="absolute top-3 -left-3 w-[55px] h-[55px] border rounded-full flex items-center justify-center bg-black text-white hover:cursor-pointer"><FaGithub className="text-2xl" /></div>
                    </Link>
                    <Link to={`${url}`}>
                    ...
                    <img src={image} alt="Image" loading="lazy" className="w-full h-full  object-contain" />
                </div>
